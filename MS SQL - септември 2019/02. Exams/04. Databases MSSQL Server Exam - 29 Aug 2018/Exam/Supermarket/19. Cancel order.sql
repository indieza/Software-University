...
AS
BEGIN
	DECLARE @targetId INT = (SELECT [o].[Id] FROM [dbo].[Orders] AS o WHERE [o].[Id] = @OrderId)
	...
	DECLARE @targetDate DATETIME = (SELECT [o].[DateTime] FROM [dbo].[Orders] AS o WHERE [o].[Id] = @OrderId)
	...
	DELETE
	  FROM [dbo].[OrderItems]
	 WHERE [dbo].[OrderItems].[OrderId] = @targetId

	DELETE
	  FROM [dbo].[Orders]
	 WHERE [dbo].[Orders].[Id] = @targetId
END
