...
BEGIN
	...
	DECLARE @targetId INT = 
		(SELECT [f].[Id] FROM [dbo].[Flights] AS f WHERE [f].[Origin] = @origin AND [f].[Destination] = @destination)
	IF(@targetId IS NULL)
	...
	DECLARE @price DECIMAL(15, 2) = (SELECT [t].[Price]
		                              ...
		                              WHERE [f].[Id] = @targetId) * @peopleCount
	
	RETURN 'Total price ' + CAST(@price AS VARCHAR(50))
END
